...
func (d *Driver) refreshAuthorization(ctx context.Context, force bool) (string, error) {
	...
	if response.StatusCode != 200 {
		return "", domain.Errorf(domain.CodeAuthExpired, "移动家庭云令牌刷新失败 HTTP %d: %s", response.StatusCode, httpx.Truncate(data, 300))
	}
	...
	if newToken == "" {
		return "", domain.Errorf(domain.CodeAuthExpired, "移动家庭云刷新成功但未返回新令牌")
	}
	prefix := info.Prefix
	if prefix == "" {
		prefix = "pc"
	...
	if err != nil {
		return "", domain.Errorf(domain.CodeAuthExpired, "移动家庭云刷新令牌格式异常: %s", err.Error())
	}
