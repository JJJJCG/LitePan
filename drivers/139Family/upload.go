...
type familyUploadResumeCtx struct {
	parentID       string
...
func (d *Driver) UploadLocalFile(ctx context.Context, req driver.LocalUploadRequest) (*driver.LocalUploadResult, error) {
	...
	resume := normalizeFamilyResumeState(req.ResumeState, parentID, requestedName, local.Size, "", len(parts))
	...
	if resume != nil && len(resume.completedParts) > 0 {
		uploaded := uploadedFamilyBytes(parts, resume.completedParts)
		uploadutil.NotifyProgress(req.OnProgress, uploaded, local.Size, "正在继续上传到移动家庭云")
	} else {
	...
	resume = normalizeFamilyResumeState(req.ResumeState, parentID, requestedName, local.Size, contentHash, len(parts))
	...
	if resume != nil {
		...
		if created.FileID.String() == "" || strings.TrimSpace(created.UploadID) == "" {
			return nil, domain.Errorf(domain.CodeDriverError, "移动家庭云上传初始化未返回 fileId 或 uploadId")
		}
		fileName = firstNonEmpty(created.FileName, fileName)
		resume = &familyUploadResumeCtx{
			parentID:       parentID,
		...
		persistFamilyResumeState(req.OnResumeState, resume, parts)
	}

	urls := make(map[int]string, len(parts))
	mergeUploadURLs(urls, created.PartInfos)
	if local.Size > 0 {
		pending := pendingFamilyParts(parts, resume.completedParts)
	...
	for _, part := range parts {
		...
		if uploadURL == "" {
			...
			return nil, domain.Errorf(domain.CodeDriverError, "移动家庭云第 %d 个分片未返回上传地址", part.PartNumber)
		}
...
// 139 家庭云上传使用动态上传接口（与个人云相同端点，但额外参数不同）。
const (
	pathUploadCreate  = "/dynamic/file/create"
	pathUploadURLs    = "/dynamic/file/getUploadUrl"
	pathUploadComplete = "/dynamic/file/complete"
)
...
func normalizeFamilyResumeState(state map[string]any, parentID, requestedName string, fileSize int64, contentHash string, totalParts int) *familyUploadResumeCtx {
...
func persistFamilyResumeState(onState driver.UploadStateCallback, resume *familyUploadResumeCtx, parts []uploadPartSpec) {
...
func pendingFamilyParts(parts []uploadPartSpec, completed map[int]struct{}) []uploadPartSpec {
...
func uploadedFamilyBytes(parts []uploadPartSpec, completed map[int]struct{}) int64 {
	var uploaded int64
	for _, part := range parts {
		if _, ok := completed[part.PartNumber]; ok {
			uploaded += part.PartSize
...
func (d *Driver) putUploadPart(ctx context.Context, localPath string, part uploadPartSpec, uploadURL string) error {
	...
	if resp.StatusCode != http.StatusOK && resp.StatusCode != http.StatusCreated && resp.StatusCode != http.StatusNoContent {
		return domain.Errorf(domain.CodeDriverError, "移动家庭云上传分片 HTTP %d: %s", resp.StatusCode, httpx.Truncate(data, 300))
	}
	return nil
}
